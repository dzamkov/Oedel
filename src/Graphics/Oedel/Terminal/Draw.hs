...
module Graphics.Oedel.Terminal.Draw (
    cursorUp,
    cursorDown,
    cursorForward,
    cursorBackward,
...
runDrawInline :: (Width -> (Height, Draw)) -> IO ()
runDrawInline drawWithWidth = do
    Just size <- Size.size
    let width = Width $ Size.width size
    let (height, draw) = drawWithWidth width
    setAppearance defaultAppearance
    replicateM_ (cells height) $ putStrLn ""
    cursorUp $ cells height
    st <- runDraw draw (width, (Width 0, Height 0), defaultAppearance)
    changeState st (width, (Width 0, height), defaultAppearance)
